...
export class SpriteStack2D {
  ...
  updateOptions(opt: Partial<SpriteStack2DOptions>): void {
    ...
    const rebuildTriggers = [
      ...
      "shadowOffsetX",
      "shadowOffsetY",
      "debugShowSingleLayer",
      "debugLayerIndex",
    ] as const;
    for (const k of rebuildTriggers)
      if (k in opt) {
        this.sourceDirty = true;
        break;
      }
  ...
  static async fromImageSource(
    src: HTMLImageElement | HTMLCanvasElement | ImageBitmap | OffscreenCanvas,
  ): Promise<RawImage> {
  ...
  private buildCarvedSlices(
    faces: RawImage[],
    hasBack: boolean,
  ): { front: RawImage[]; back: RawImage[] } {
    const FI = [0, 1, 2, 3] as const;
    const top = faces[FI[0]],
      front = faces[FI[1]],
      side = faces[FI[2]],
      back = faces[FI[3]];
    const w = top.width,
    ...
    for (let z = 0; z < h; z++) {
      ...
      for (let y = 0; y < d; y++) {
        ...
        let lo = -1;
        for (let x = 0; x < w; x++) {
          if (mask[row + x] === 0) lo = x;
          else distL[row + x] = x - lo;
        }
        lo = w;
        for (let x = w - 1; x >= 0; x--) {
          if (mask[row + x] === 0) lo = x;
          else distR[row + x] = lo - x;
        }
      }
      for (let x = 0; x < w; x++) {
        let lo = -1;
      ...
      for (let y = 0; y < d; y++) {
        ...
        for (let x = 0; x < w; x++) {
          ...
          let done = false;
          if (sideOk && (distL[i] < distF[i] || distR[i] < distF[i]))
            done = putPixel(buf, di4, sd, s4);
          if (!done && useFront) done = putPixel(buf, di4, fd, f4);
          if (!done && sideOk) done = putPixel(buf, di4, sd, s4);
          if (!done) putPixel(buf, di4, td, di4);
          done = false;
          if (sideOk && (distL[i] < distB[i] || distR[i] < distB[i]))
            done = putPixel(bufBack, di4, sd, s4);
  ...
  private canvasSize(): [number, number] {
    ...
    const w = this.sliceImages[0].width,
      d = this.sliceImages[0].height;
  ...
  private blitPairs(
    ...
    for (let k = 0; k < pairs.length; k += 2) {
      ...
      if (a === 255) {
        frame[di4] = src[si4];
        frame[di4 + 1] = src[si4 + 1];
        frame[di4 + 2] = src[si4 + 2];
        frame[di4 + 3] = 255;
      } else {
        const inv = 255 - a;
  ...
  private boxBlur(
    ...
    for (let o = 0; o < outer; o++) {
      ...
      for (let k = 0; k < inner; k++) {
        dst[idx(o, k)] = sum * inv;
        sum +=
          src[idx(o, clamp(k + radius + 1, 0, inner - 1))] -
  ...
  private rasterShadow(
    ...
    for (let k = 0; k < pairs.length; k += 2) {
      ...
      frame[di4 + 1] = rgb[1];
      frame[di4 + 2] = rgb[2];
      frame[di4 + 3] = Math.round((a * shadowA) / 255);
    }
  }
}
